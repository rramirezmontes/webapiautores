    ...
    public class HATEOASAutorFilterAttribute : HATEOASFiltroAttribute
    {
                ...
                ResultExecutionDelegate next)
        {
            ...
            if (autorDTO == null)
            {
                ...
                autoresDTO.ForEach(async autor => await generadorEnlaces.GenerarEnlances(autor));
                resultado.Value = autoresDTO;
            }
            else {                                                                                                  
                await generadorEnlaces.GenerarEnlances(autorDTO);
            }
           /* var modelo = resultado.Value as AutorDTO ?? throw new
                            ArgumentNullException("Se esperaba una instanacia de AutorDTO");*/
